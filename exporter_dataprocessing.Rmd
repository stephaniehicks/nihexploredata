...
if(!file.exists(here("data_raw", "nih_data.RDS"))){
  ...
  tmp <- read_csv(zip_files[1])
  
  download.file(zip_files[1], destfile = here("data_raw"))
  csv_files <- list.files(here(), ".csv")
```


Things below this need to be edited.
```{r}


# files <- dir(recursive = T, pattern = ".csv")
# reporter_data <- files %>% 
#   map_dfr(read_csv, col_types = cols(.default = col_character()))
# reporter_data %>% saveRDS("RePORTER_PRJ_FY_2015_2020.RDS")

reporter_data <- readRDS(file = "RePORTER_PRJ_FY_2015_2020.RDS")

write_csv(reporter_data, "nih.csv")
```
